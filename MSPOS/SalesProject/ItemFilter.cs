                ...
                if (passingvalues.chckvalues == "1")
                {
                    if (txtStock.Text.Trim() == "Stock Items" || txtStock.Text.Trim() == "All" || txtStock.Text.Trim() == "Negative Stock" || txtStock.Text.Trim() == "Zero Stock" || txtStock.Text.Trim() == "Non Negative Stock")
                    {
                        ...
                        else if (txtStock.Text.Trim() == "Stock Items")
                        {
                           ...
                           // str = "select item_table.Item_no As Item_code,item_table.Item_name,isnull(sum(nt_qty),0) AS nt_cloqty,item_table.item_cost as item_cost,'0' as tot from stktrn_table,item_table where stktrn_table.strn_type in(0,3,12,2) and ";
                            str = "select item_table.Item_code,item_table.Item_name,isnull((nt_cloqty),0) AS nt_cloqty,item_table.item_cost as item_cost,'0' as tot from stktrn_table,item_table where stktrn_table.strn_type in(0,3,2) and ";
                            //only Getting sales qty 
                            //strn1_salesqty = "select item_table.Item_no As Item_code,item_table.Item_name,isnull(sum(nt_qty),0) AS nt_cloqty,item_table.Item_cost as item_cost,'0' as tot from stktrn_table,item_table where  stktrn_table.strn_type in(1,11) and stktrn_table.strn_type<>2 and ";
                            strn1_salesqty = "select item_table.Item_code,item_table.Item_name,isnull(sum(nt_qty),0) AS nt_cloqty,item_table.Item_cost as item_cost,'0' as tot from stktrn_table,item_table where  stktrn_table.strn_type in(1,11) and stktrn_table.strn_type<>2 and ";
                        }
                    }
                }
