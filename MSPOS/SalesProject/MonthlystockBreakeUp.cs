...
namespace SalesProject
{
    public partial class MonthlystockBreakeUp : Form
    {
        ...
        public MonthlystockBreakeUp()
        {
            ...
            //SqlCommand cmd = new SqlCommand("select * from item_table where Item_name=@ItemCode", con);
            SqlCommand cmd = new SqlCommand("select * from item_table where Item_code=@ItemCode", con);
            //SqlCommand cmd = new SqlCommand("SP_SelectQuery", con);
            //cmd.CommandType = CommandType.StoredProcedure;

            //cmd.Parameters.AddWithValue("@ActionType","ItemCode");
            cmd.Parameters.AddWithValue("@ItemCode", id_number.ToString().Trim());
                        ...
                        if (e.RowIndex != 13)
                        {
                            ...
                            if (e.RowIndex.Equals(0))
                            {
                                ...
                                if (e.RowIndex != 0 != null)
                                {
                                    ...
                                    {
                                         //MSPOSBACKOFFICE.passingvalues.tAmountType = "Gross Amount";
                                        
                                       //Call to Item Ledger Code:
        ...
        public void itemcreationformload()
        {
            ...
            if (currentForm == null)//if form not found
            {
                //MSPOSBACKOFFICE.ItemCreations frm1 = new MSPOSBACKOFFICE.ItemCreations(itemnumberbassing);
                //frm1.MdiParent = this.ParentForm;
                //frm1.StartPosition = FormStartPosition.Manual;
                //frm1.WindowState = FormWindowState.Normal;
                //frm1.Location = new Point(0, 80);
