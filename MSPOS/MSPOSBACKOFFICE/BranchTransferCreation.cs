...
{
    public partial class BranchTransferCreation : Form
    {
        ...
        public void countload()
        {
            ...
            SqlCommand cmd = new SqlCommand("select branch_name from branch_table order by branch_name ASC", con);
            SqlDataAdapter adp = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            dt.Rows.Clear();
            lst_ctrname.Items.Clear();
            adp.Fill(dt);
            //MessageBox.Show(_Class.clsVariables.tBr_Name.ToString());
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (_Class.clsVariables.tBr_Name.ToString() != dt.Rows[i]["branch_name"].ToString())
                    {
                        lst_ctrname.Items.Add(dt.Rows[i]["branch_name"].ToString());
                        //txt_countername.Text = (dt.Rows[0]["branch_name"].ToString());
                        this.lst_ctrname.SelectedIndex = 0;
                    }
                }
            }
            txt_countername.Text = lst_ctrname.Items[0].ToString();
            adp.Dispose();
        }
        #endregion
        ...
        private void BranchTransferCreation_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (txt_comp_name.Text != "" || rowcount != 0)
                {
                    ...
                    if (myDataGrid1.Rows[0].Cells["Code"].Value.ToString() != "" || myDataGrid1.Rows[0].Cells["Name"].Value.ToString() != "")
                    {
                        ...
                        SqlCommand cmd = new SqlCommand("sp_BranchTransferCreate", con);
                        ...
                        cmd.Parameters.AddWithValue("@tBr_Name", _Class.clsVariables.tBr_Name.ToString());
                        cmd.ExecuteNonQuery();
                        con.Close();
                        dt1.Rows.Clear();
        ...
        public void amountupdate()
        {
            con.Close();
            con.Open();
            SqlCommand cmd = new SqlCommand("sp_AmountupdateBranch", con);
            cmd.Transaction = trans;
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@tCtr_name", txt_countername.Text);
        ...
        public void qtyupdate()
        {
            try
            {
                ...
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@temp_Table", dt1);
                cmd.Parameters.AddWithValue("@tBr_Name", lst_ctrname.SelectedItem.ToString());
        ...
        private void myDataGrid1_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (myDataGrid1.CurrentCell.ColumnIndex == 2)
            {
                ...
                //myDataGrid1.CurrentCell = myDataGrid1[myDataGrid1.CurrentCell.ColumnIndex + 1, myDataGrid1.CurrentCell.RowIndex];
                //int rw=myDataGrid1.CurrentCell.RowIndex;
                //int cel=myDataGrid1.CurrentCell.ColumnIndex+3;
                //myDataGrid1.Rows[rw].Cells[cel].Selected = true;
            }
        ...
        private void BranchTransferCreation_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape)
            {
                this.Close();
        ...
        private void txt_countername_TextChanged(object sender, EventArgs e)
        {
            if (txt_countername.Text.Trim() != null)
            {
                if (txt_countername.Text.ToString().Trim() != "")
                {
                    ...
                    SqlCommand cmd = new SqlCommand("select branch_name from branch_table where branch_name like '" + txt_countername.Text.Trim() + "%'", con);
                    SqlDataAdapter adp = new SqlDataAdapter(cmd);
                    DataTable dt = new DataTable();
                    dt.Rows.Clear();
                    adp.Fill(dt);
                    ...
                    {
                        isChk = true;
                        string tempStr = dt.Rows[j]["Branch_Name"].ToString();
                        for (int i = 0; i < lst_ctrname.Items.Count; i++)
                        {
                            if (dt.Rows[j]["Branch_Name"].ToString() == lst_ctrname.Items[i].ToString())
                            {
