...
namespace MSPOSBACKOFFICE
{
    ...
    public partial class frmLedgerCreation : Form
    {
        ...
        public event EventHandler funLedgerLoadEventClick;
           ...
           (0,0,'',0,'','','',@tCtr_no,@tUserNo,0,0,0,0,'2014-03-31',255,@tledger_no,0,@tDr_amount,@tCr_amount,'' ,'',0,'',0)", con);
                                    ...
                                    else if (LedgerNumber.ToString().Trim() != string.Empty)
                                    {
                                        ...
                                        if (tExexute == "Execute")
                                        {
                                            ...
                                            cmdUpdateLedger.Parameters.AddWithValue("@DOB", txtDOB.Text.ToString().Trim() == "" ?Convert.ToDateTime("01/01/9999") : Convert.ToDateTime(txtDOB.Text.ToString().Trim()));
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(LedgerNumber))
                {
                    DataTable dtDeleteChk = new DataTable();
                    dtDeleteChk.Rows.Clear();
                    SqlCommand cmd = new SqlCommand(@"if exists( Select party_no from salmas_table where party_no=@Ledgerno)
begin
Select 'Dont Delete';
end
else if exists( Select SalRecv_Led from SalRecv_table where SalRecv_Led=@Ledgerno)
begin
Select 'Dont Delete';
end
else if exists( Select Ledger_no from PaymentDetail_table where Ledger_no=@Ledgerno)
begin
Select 'Dont Delete';
end
else if exists( Select PaymentLedger_No from PaymentDetail_table where PaymentLedger_No=@Ledgerno)
begin
Select 'Dont Delete';
end
else if exists( Select party_no from purmas_table where party_no=@Ledgerno)
begin
Select 'Dont Delete';
end
else
...
Select 'Delete';
                    ...
                    cmd.Parameters.AddWithValue("@Ledgerno", LedgerNumber);
                    SqlDataAdapter adp = new SqlDataAdapter(cmd);
                    adp.Fill(dtDeleteChk);
                    if (dtDeleteChk.Rows.Count > 0)
                    {
                        if (Convert.ToString(dtDeleteChk.Rows[0][0]).ToUpper() == "DELETE")
                        {
                            SqlCommand cmdDelete = new SqlCommand("Delete from Ledger_table where Ledger_no=@Ledgerno", con);
                            cmdDelete.Parameters.AddWithValue("@Ledgerno", LedgerNumber);
                            cmdDelete.ExecuteNonQuery();
                            Clear();
                            if (funLedgerLoadEventClick != null)
                            {
                                funLedgerLoadEventClick(sender,e);
                            }
                        ...
                        {
                            MyMessageBox.ShowBox("This Ledger value cannot remove from the list", "Warning");
                        }
                    }
                }
            }
